...
- **CI**: `bundle exec rake` (rspec + rubocop) now runs on every push/PR via GitHub Actions across Ruby 3.0/3.1/3.2/3.3 (#6).
...
- **Tooling**: codebase now lints with RuboCop (`.rubocop.yml` + tracked `.rubocop_todo.yml`); removed unused `vcr` development dependency from `Gemfile` (the spec suite is WebMock-only).
